    ...
    stages {
        ...
        stage('Publish to Nexus'){
            steps{
                ...
                sh 'curl -v -F r=devopstraining -F hasPom=true -F e=jar -F g=comrades.aea -F a=Calc -F v=5.0 -F c=ArithmeticCalculator -F p=jar -F file=@target/Calc-CALC-1.0.jar -u ${username}:${password} http://18.224.155.110:8081/nexus/content/repositories/devopstraining/comrades'
