    ...
    stages {
        ...
        stage('Publish to Nexus'){
            steps{
                ...
                curl -v -F r=devopstraining -F hasPom=true -F e=war -F g=comrades.aea -F a=Calc -F v=5.0 -F c=ArithmeticCalculator -F p=war -F file=@**/target/*.war -u trainee:trainee http://18.224.155.110:8081/nexus
            }
        }
    }
}
